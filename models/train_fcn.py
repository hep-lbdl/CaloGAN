...
if __name__ == '__main__':
    ...
    # Ravel all of the calo images together
    h = concatenate([
        Flatten()(calorimeter[l]) for l in range(3)
    ], axis=-1)

    energies = concatenate([
        calculate_energy(calorimeter[l]) for l in range(3)
    ], axis=-1)
    ...
    def slicer(x, slc):
        return Lambda(lambda x: x[..., slc])(x)

    def dense_generator(x):

        list_of_shapes = [(3, 96), (12, 12),  (12, 6)]
        spec = [(0, 288), (288, 432), (432, 504)]
