...
if __name__ == '__main__':
    ...
    def _load_data(particle, datafile):
        ...
        y = [particle] * energy.shape[0]

        d.close()

        return data, energy, y
    ...
    data, y, energy = shuffle(data, y, energy,
                                              random_state=0)

    logger.info('Building discriminator')

    calorimeter = [Input(shape=(25, 25, 1)) for i in range(25)]
    ...
    # linear last layer
    lagan_generators = [build_generator(h, 25, 25)] * 25 
    #img_layer0 = build_generator(h, 3, 96)
    output_images = [lagan_generators[0]]
    if not no_attn:

        logger.info('using attentional mechanism')
        for i in range(1, 25):

            output_images.append(inpainting_attention(lagan_generators[i], output_images[i-1]))

    generator_outputs = [
        Activation('relu')(img) for img in output_images
    ]
    ...
    for epoch in range(nb_epochs):
        ...
        nb_batches = int(data.shape[0] / batch_size)
        if verbose:
        ...
        for index in range(nb_batches):
            ...
            data_images = np.split(data, 25, axis=3)
            data_images = [np.expand_dims(np.squeeze(a), -1) for a in data_images]
            image_batch = [di[index * batch_size:(index + 1) * batch_size] for di in data_images]
